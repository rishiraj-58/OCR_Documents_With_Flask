...
def recognise_text(image_path):
    ...
    for coords in coordinates:
        ...
        # cropping image based on the coordinates
        temp = image.crop((x, y, w, h))

        # binarizing image
        thresh = temp.point(lambda x: 255 if x > 100 else 0, '1')

        # padding the image with 10 pixels for better prediction with tesseract
        thresh = Image.new('L', (thresh.size[0]+20, thresh.size[1]+20), 255)
        thresh.paste(thresh, (10, 10))
